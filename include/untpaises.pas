  ...
  with formPrincipal do
    ...
    try
      ...
      SQL.Text :=
        ...
        'Países p                                                            ' +
        'LEFT JOIN                                                           ' +
        'Competicoes C ON C.País = p.País                                    ' +
        'LEFT JOIN                                                           ' +
        'Jogo j ON j.Cod_Comp = C.Cod_Comp                                   ' +
        'LEFT JOIN                                                           ' +
        'Times T ON T.Time = j.Mandante OR J.Visitante                       ' +
        'LEFT JOIN                                                           ' +
  ...
  with formPrincipal do
    if MessageDlg('Remover País', 'Tem certeza que deseja remover o país selecionado?'
      ...
      begin
        try
          DataBase := conectBancoDados;
          SQL.Text := 'SELECT P.País, C.Competicao FROM Países P ' +
            'LEFT JOIN Competicoes C ON C.País = P.País ' + 'WHERE P.País = :pais';
          ParamByname('pais').AsString := qrPais.FieldByName('País').AsString;
          Open;
          if IsEmpty then
            raise Exception.Create('País selecionado já está registrado em uma ' +
              'competição. Favor remover competição(ões) referente(s) e tente novamente');
